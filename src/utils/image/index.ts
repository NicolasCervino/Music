function ensureColorContrast(color: string): string {
   ...
   if (luminance > 0.8) {
      ...
      // If dark, brighten more aggressively
...
function hexToRgb(hex: string) {
   ...
   if (hex.length === 8) {
      hex = hex.substring(0, 6); // Remove alpha component for luminance calculations
   }
...
function rgbToHex({ r, g, b }: { r: number; g: number; b: number }): string {
   ...
   return `#${validR.toString(16).padStart(2, '0')}${validG.toString(16).padStart(2, '0')}${validB
      .toString(16)
      .padStart(2, '0')}`;
}

export { ensureColorContrast, hexToRgb, lightenColor };
